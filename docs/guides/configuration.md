...
| `cache_lock_timeout` | `clangquill_cache_lock_timeout` | `300.0` | Seconds an incremental build waits for another build already holding the single-writer lock on `cache_dir` before giving up. Only one build may run against a given `cache_dir` at a time; concurrent readers are unaffected. See [concurrent builds](../usage.md#concurrent-builds). Ignored when `cache_dir` is `None`. |
